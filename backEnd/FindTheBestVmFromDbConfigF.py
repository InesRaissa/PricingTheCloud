...
def find_TheBestVm_in_db(vm):    
    ...
    try:
        ...
        if vm.cpu is not None:
            ...
            values.append(vm.cpu)
        if vm.memory is not None:
            #int(memory)
            conditions.append("memory_vm = %s")
            values.append(vm.memory)
        if vm.csp is not None:
            conditions.append("csp_vm = %s")
            values.append(vm.csp)
        ...
        if len(results)==0:
            chosen_vm=None
            raise Exception(f"Sorry, no vms with {vm.cpu} GB cpu and {vm.memory} GB memory in {vm.csp}'s data bases")
        else:
            ...
            print("parameters of the VM selected from DB:")
